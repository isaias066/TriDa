...
export interface SortConfig<T> {
  /** Clave del campo por el que se ordena. */
...
export interface DataTableColumn<T> {
  /** Clave única de la columna (usada para sort). */
  key: string;
  /** Etiqueta visible en el header. */
  label: ReactNode;
  /** Función que renderiza el contenido de la celda. */
  render: (item: T, index: number) => ReactNode;
  /** Si es true, permite ordenar por esta columna. */
  sortable?: boolean;
  /** Función personalizada de sort (si el default no aplica). */
  sortAccessor?: (item: T) => string | number | Date | null | undefined;
  /** Alineación del contenido. Default: 'left'. */
  align?: 'left' | 'center' | 'right';
  /** Ancho de la columna (ej: '150px', '20%', 'auto'). */
  width?: string;
  /** Ancho mínimo. */
  minWidth?: string;
  /** Estilo adicional del header. */
  headerStyle?: CSSProperties;
  /** Estilo adicional de la celda. */
...
export interface DataTableProps<T> {
  ...
  /** Función que retorna la key única de cada fila. */
  getRowKey: (item: T, index: number) => string | number;
  /** Configuración actual de sort. */
  sort?: SortConfig<T>;
  /** Callback al cambiar el sort. */
  onSortChange?: (sort: SortConfig<T>) => void;
  /** Callback al hacer click en una fila. */
  onRowClick?: (item: T, index: number) => void;
  /** Item actualmente seleccionado (se resalta). */
  selectedRow?: T | null;
  /** Función personalizada para comparar filas (default: identidad). */
  isRowSelected?: (item: T) => boolean;
  /** Si es true, muestra el spinner de carga. */
  loading?: boolean;
  /** Mensaje personalizado cuando no hay datos. */
  emptyMessage?: string;
  /** Componente personalizado cuando no hay datos. */
  emptyComponent?: ReactNode;
  /** Función para agregar clases dinámicas a las filas. */
  getRowClassName?: (item: T, index: number) => string;
  /** Función para agregar estilos dinámicos a las filas. */
  getRowStyle?: (item: T, index: number) => CSSProperties;
  /** Si es true, la tabla tiene bordes visibles entre filas. Default: true. */
  bordered?: boolean;
  /** Si es true, las filas tienen hover effect. Default: true. */
  hoverable?: boolean;
  /** Si es true, aplica sort automáticamente. Default: false (sort externo). */
  autoSort?: boolean;
  /** Clase CSS adicional. */
  className?: string;
}
...
/**
 * Ordena un array según la configuración de sort.
 */
function sortData<T>(
  data: T[],
  sort: SortConfig<T>,
...
/**
 * Tabla genérica con sort, selección y rendering personalizable.
 *
 * @example
 * // Uso básico
 * <DataTable
 ...
 *   getRowKey={(tx) => tx.id}
 *   columns={[
 *     { key: 'id', label: 'ID', render: (tx) => tx.id, sortable: true },
 *     { key: 'user', label: 'Usuario', render: (tx) => tx.user, sortable: true },
 *     { key: 'amount', label: 'Monto', render: (tx) => formatCOP(tx.amount), align: 'right' },
 *   ]}
 * />
 *
 * @example
 * // Con sort externo
 * const [sort, setSort] = useState<SortConfig<Transaction>>({
 *   field: 'timestamp',
 *   direction: 'desc',
 * });
 *
